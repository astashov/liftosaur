...
export class LlmUtil {
  ...
  private async initializeProvider(): Promise<void> {
    ...
    // Try Claude first for better quality
    try {
      const anthropicKey = await this.secrets.getAnthropicKey();
      if (anthropicKey) {
        this.log.log("Using Claude provider for LLM");
        this.provider = new ClaudeProvider(anthropicKey);
        return;
      }
    } catch (e) {
      this.log.log("No Anthropic key found:", e);
    }

    // Fallback to OpenAI
    const openaiKey = await this.secrets.getOpenAiKey();
    if (openaiKey) {
      this.log.log("Using OpenAI provider for LLM");
      this.provider = new OpenAIProvider(openaiKey);
    } else {
      throw new Error("No API key found for Claude or OpenAI");
    }
  }
  ...
  public async *convertProgramToLiftoscriptStream(
    input: string
  ): AsyncGenerator<{ type: "progress" | "result" | "error" | "retry" | "finish"; data: string }, void, unknown> {
    // Initialize provider if needed
    await this.initializeProvider();
    if (!this.provider) {
      yield { type: "error", data: "Failed to initialize LLM provider" };
      return;
    }
    ...
    try {
      for await (const event of this.provider.convertProgramToLiftoscriptStream(programContent)) {
        yield event;
      }
    } catch (error) {
