...
export function CollectionUtils_repeat<T>(el: T, length: number): T[] {
  const arr: T[] = [];
  for (let i = 0; i < length; i += 1) {
    arr.push(el);
  }
...
export function CollectionUtils_groupByKey<
  T,
  K extends keyof T,
...
export function CollectionUtils_groupByKeyUniq<
  T,
  K extends keyof T,
  U extends T[K] extends string ? string : T[K] extends number ? number : never,
>(arr: T[], key: K): Partial<Record<U, T>> {
...
export function CollectionUtils_remove<T>(from: T[], item: T): T[] {
  return from.filter((t) => t !== item);
}

export function CollectionUtils_removeAll<T>(from: T[], item: T[]): T[] {
  return from.filter((t) => !item.includes(t));
}

export function CollectionUtils_removeAt<T>(from: T[], index: number): T[] {
  const result = [...from];
...
export function CollectionUtils_nonnull<T>(from: (T | undefined)[]): T[] {
...
export function CollectionUtils_merge<T>(from: T[], to: T[]): T[] {
