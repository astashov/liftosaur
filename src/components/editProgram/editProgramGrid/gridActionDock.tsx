...
export const GridActionDock = memo(function GridActionDock(): JSX.Element | null {
  ...
  return (
    ...
    >
      {/* Only the source that lies further down: one that is above belongs at the top of the
          screen, under the week header, and the grid renders it there. */}
      {props.reuse?.direction === "down" && (
        <View className="pb-1">
          <GridReusePillFloat reuse={props.reuse} />
        </View>
      )}
