    ...
    >
      ...
      >
        ...
        {props.values.map(([value, label], index) => (
          <button
            key={value}
            data-cy={`scroll-barrel-item-${StringUtils_dashcase(label)}`} data-testid={`scroll-barrel-item-${StringUtils_dashcase(label)}`} testID={`scroll-barrel-item-${StringUtils_dashcase(label)}`}
