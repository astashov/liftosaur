...
export function ProgramContentEditWeeks(props: IProgramContentEditWeeksProps): JSX.Element {
  ...
  return (
    <>
      <GroupHeader leftExpandIcon={true} size="large" name="Weeks" isExpanded={true}>
        <DraggableList
          hideBorders={true}
          items={program.weeks}
          ...
          onClick={() => {
            dispatch(
              lbProgram.p("weeks").recordModify((weeks) => {
                return [
                  ...weeks,
                  Program.createWeek(StringUtils.nextName(program.weeks[program.weeks.length - 1].name)),
                ];
              })
            );
          }}
      ...
      <GroupHeader size="large" name="Days" />
    </>
  );
}
