...
export function PlannerDay(props: IPlannerDayProps): JSX.Element {
  ...
  const [showProgramDescription, setShowProgramDescription] = useState(!!day.description?.trim());
  ...
  return (
    <div className="flex flex-col md:flex-row">
      <div className="flex-1">
        ...
        {props.lastDayDescription && !props.day.stopRepeatingDescription && (
          <>
            <div>
              <Markdown value={props.lastDayDescription} />
            </div>
            <div>
              <LinkButton
                className="text-xs"
                name="planner-day-stop-repeating-description"
                onClick={() => {
                  dispatch(
                    lbProgram.p("weeks").i(weekIndex).p("days").i(dayIndex).p("stopRepeatingDescription").record(true)
                  );
                }}
              >
                Stop Repeating Description
              </LinkButton>
            </div>
          </>
        )}
        {showProgramDescription ? (
          ...
          <div>
            <LinkButton
              ...
              onClick={() => setShowProgramDescription(true)}
            >
              Add Day Description
            </LinkButton>
          </div>
