...
export function ProgramDetailsGzclUhf5w(props: IProgramDetailsGzclUhf9wProps): JSX.Element {
...
function buildStaticStates(
  ...
  return program.exercises.reduce<Partial<Record<string, IProgramState>>>((acc, exercise) => {
    if (day.exercises.map((e) => e.id).indexOf(exercise.id) !== -1) {
      ...
      const entry = Program.programExerciseToHistoryEntry(exercise, program.exercises, dayData, settings, {
        week,
      });
